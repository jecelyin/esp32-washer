...
#define ENABLE_TONE 1
  ...
  {
#ifdef ENABLE_TONE
    tone(BUZZER_PIN, NOTE_F7, 500);
    delay(500);
    tone(BUZZER_PIN, NOTE_FS7, 500);
    delay(500);
    tone(BUZZER_PIN, NOTE_G7, 1500);
    delay(1500);
    noTone(BUZZER_PIN);
#endif
  }
  ...
  void error()
  {
    for (int i = 0; i < 10; i++)
    {
      tone(BUZZER_PIN, 1000, 100); // 播放1000Hz音符，持续100毫秒
      delay(100);                  // 等待100毫秒

      tone(BUZZER_PIN, 500, 100); // 播放500Hz音符，持续100毫秒
      delay(100);                 // 等待100毫秒
    }

    noTone(BUZZER_PIN); // 停止音符播放
  }
}